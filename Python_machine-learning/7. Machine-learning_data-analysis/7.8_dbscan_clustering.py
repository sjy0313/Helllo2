...
# 경계 포인트 : 반지름 R안에 다른 코어 포인트가 있을 경우
# Noise(outlier) : 코어/경계 포인트에도 속하지 않는 점.
# 하나의 cluster는 반지름 R안에 서로 위치하는 모든 코어 포인트를 포함하는 방식으로 구성
# 각 코어 포인트 주위에 있는 경계 포인트도 cluster에 포함. 
