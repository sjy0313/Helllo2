...
hat = pd.read_csv('dataset/Edu_infra.csv') # hat 변수에 데이터셋 입력
# 30만 평 당 1제곱 킬로미터
...
# chick: 부화된 병아리(마릿수)
"""
  시/도    유치원   초등학교    중학교   고등학교  ...    대학  행정복지센터  취업위탁기관     총합계        면적
0  대전  249.0  149.0   89.0   62.0  ...  18.0    81.0    12.0  1501.0  539.67k㎡
1  세종   65.0   53.0   27.0   21.0  ...   2.0    24.0     3.0   400.0  464.92k㎡
2  충남  495.0  414.0  185.0  118.0  ...  25.0   208.0    26.0  2776.0  8,247.5㎢
3  충북  317.0  255.0  128.0   84.0  ...  18.0   153.0    18.0  1804.0    7,407㎢
"""
...
# In[22]:
# 전체 중 각각의 비율 구하기
pct = hat['유치원']/hat['유치원'].sum()
pct


# In[23ArithmeticError


# 파이차트 그리기
col7 = sns.color_palette('Pastel2', 7)
plt.figure(figsize=(10, 10))
plt.pie(pct, labels = hat['시/도'], autopct='%.1f%%', colors = col7, startangle = 0, counterclock = False)
plt.title('충청도 유치원 수 현황', fontsize =17)
plt.show()
