...
# In[4]:
# loc: 인덱스 라벨을 기반으로 행과 열을 선택/ iloc: 정수 위치를 기반으로 행과 열을 선택
# 그래서 보통 인덱스 열이 0~ 로 df상에 설정되어있으면 iloc에서 슬라이싱으로 위치를 참조가능
# loc[행선택, 열선택]
# 인덱스 : 0,1 행
# 컬럼:  ['bookname','authors']
...
b1 = books_df.loc[0:1, 'bookname':'authors']
...
# isbn은 변수로, URL의 특정 부분을 동적으로 만들기 위해 사용
# r : models.Response 
