...
# ns_book7의 df size를 확인해 보면 376770이 나온다. 이는 데이터 정제단계에서 도서권수가 양수인 행이 중복된 경우가
# 포함되어서 그런 것.

ns_book7 = ns_book6[ns_book6['도서권수'] > 0].drop_duplicates() # 376770
# drop.duplicates()로 중복된 데이터를 하나로 보고 계산했더니 결과값의 계산이 잘 되었다. 
...
B 데이터 경우 1.5 3 4.5 / C 데이터의 경우 0.15 0.3 0.45 앞에 3개의 값들 사이에 데이터들의 대부분이
# 분포해 있다고 판단할 수 있다. 
...
# 나의 질문: 
'''
따라서 표본의 크기가 30정도 되면 자유도도 n이 되며 numpy 에서 분산을 구할 떄 
ddof매서드를 따로 지정하지 않아도 된다는 말이죠?

# 유념해야 할 사실
통계학에서는 표본의 크기(n)가 30 이상이면 중심 극한 정리를 충분히 따르고 있다고 보는 경우가 많습니다.
 이는 일반적인 가이드라인일 뿐이며, 실제로는 분석하고자 하는 데이터나 문제에 따라 다를 수 있습니다.
 
ChatGPT
네, 일반적으로 표본의 크기가 30 이상이면 중심 극한 정리에 따라 
표본 평균의 분포가 정규 분포에 근접하게 되며, 이에 따라 자유도가 n으로 가까워집니다. 
따라서 이런 경우에는 표본의 크기가 자유도와 거의 동일하게 됩니다.

numpy의 std() 함수는 기본적으로 ddof=0으로 설정되어 있습니다. 
따라서 표본의 크기가 30 이상인 경우에는 별도로 ddof 매개 변수를 지정하지 않아도 됩니다. 
그러나 특정한 경우에는 표본의 크기에 상관없이 ddof 매개 변수를 명시적으로 지정하는 것이 좋을 수 있습니다.
'''

# pandas : 넘파이 방식, n
ns_book7['대출건수'].var(ddof=0) # 371.6946438971496
# ddof 매서드로 자유도의 차감값을 지정할 수 있는데 판다스 매서드의 ddof의 기본값은 1이므로 n-1로 계산된다
# 하지만 ns_book7의 데이터가 
'''
표본의 크기가 충분히 크면(보통 30 이상), 중심 극한 정리에 따라 표본의 분포가 정규 분포에 근접하게 되고,
이에 따라 표본의 분산을 추정할 때 Bessel's correction(베셀 교정)을 적용하지 않아도 됩니다. 따라서
표본의 크기가 크면 표본의 분산을 추정할 때 ddof=0을 사용할 수 있습니다.
'''
# In[42]:

# numpy : 판다스 방식, n-1
np.var(ns_book7['대출건수'], ddof=1) #  371.69563042906674
# 판다스에서 넘파이 var()함수는 ddof의 기본값은 0이므로 n으로 나눗셈을 한다

'''
std()도 마찬가지로 ddof 의 기본값이 0으로     
이는 표본을 대상으로 표준 편차를 계산할 때 표본의 자유도를 고려하여 편향을 조정한다는 것을 의미합니다. 
'''
